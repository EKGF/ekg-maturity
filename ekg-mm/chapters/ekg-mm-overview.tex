\pagebreak
\chapter{Overview}\label{ch:ekg-mm-overview}

Without showing the levels of the maturity model, here is an overview of the taxonomy of pillars, components,
capabilities, and the measurable abilities or summaries.

%
% Most of the rest of this file has been copied & pasted from the Excel spreadsheet
% that MIA is maintaining, via the Tables Generator website 
% at https://www.tablesgenerator.com/latex_tables#
%
% So there is no need to manually maintain this file, unless we decide to make
% everything in this table a link and give up the spreadsheet.
%
% The following changes have been made to the generated output:
%
% 1. Remove "\color[HTML]{FFF} " which causes issues. 
% 2. Insert the line \scriptsize just after \begin{table}[]
% 3. In the first column, change text like "\textbf{A. Business}" into
%    "\rotatebox{90}{\textbf{A. Business}}"
%
\bgroup

\newcommand{\setComponentColor}[1]{
    \colorlet{componentColor}{#1}
}

\newcommand{\componentLabel}[2]{
    \cellcolor{#1}\textbf{\ref{#2}~\nameref{#2}}
}

\newcommand{\labelCB}[2]{%
    \ifnum#1=1%
        \componentLabel{componentBusinessColor}{ch:ekg-mm-a-#2}%
    \else%
        \multirow{-#1}{*}{\componentLabel{componentBusinessColor}{ch:ekg-mm-a-#2}}%
    \fi%
}

\newcommand{\labelCD}[2]{%
    \ifnum#1=1%
    \componentLabel{componentDataColor}{ch:ekg-mm-b-#2}%
    \else%
    \multirow{-#1}{*}{\componentLabel{componentDataColor}{ch:ekg-mm-b-#2}}%
    \fi%
}

\newcommand{\labelCT}[2]{%
    \ifnum#1=1%
    \componentLabel{componentTechnologyColor}{ch:ekg-mm-c-#2}%
    \else%
    \multirow{-#1}{*}{\componentLabel{componentTechnologyColor}{ch:ekg-mm-c-#2}}%
    \fi%
}

\newcommand{\labelCO}[2]{%
    \ifnum#1=1%
    \componentLabel{componentOrganizationColor}{ch:ekg-mm-d-#2}%
    \else%
    \multirow{-#1}{*}{\componentLabel{componentOrganizationColor}{ch:ekg-mm-d-#2}}%
    \fi%
}

\newcommand{\setPillarColor}[1]{
    \def\pillarColor{#1}
}

\newcommand{\pillarLabel}[2]{
    \cellcolor{#1}{\rotatebox{90}{\fontsize{6.5pt}{8pt}\selectfont\textbf{\ref{#2}~\nameref{#2}}}}
}

\newcommand{\labelPB}[2]{\multirow{-#1}{*}{\pillarLabel{pillarBusinessColor}{pt:ekgmm-#2}}}
\newcommand{\labelPD}[2]{\multirow{-#1}{*}{\pillarLabel{pillarDataColor}{pt:ekgmm-#2}}}
\newcommand{\labelPT}[2]{\multirow{-#1}{*}{\pillarLabel{pillarTechnologyColor}{pt:ekgmm-#2}}}
\newcommand{\labelPO}[2]{\multirow{-#1}{*}{\pillarLabel{pillarOrganizationColor}{pt:ekgmm-#2}}}


\def\cellPA{\cellcolor{pillarBusinessColor}}
\def\cellPB{\cellcolor{pillarDataColor}}
\def\cellPC{\cellcolor{pillarTechnologyColor}}
\def\cellPD{\cellcolor{pillarOrganizationColor}}

\def\cellCB{\cellcolor{componentBusinessColor}}
\def\cellCD{\cellcolor{componentDataColor}}
\def\cellCT{\cellcolor{componentTechnologyColor}}
\def\cellCO{\cellcolor{componentOrganizationColor}}

\def\cellKA{\cellcolor{capabilityBusinessColor}}
\def\cellKB{\cellcolor{capabilityDataColor}}
\def\cellKC{\cellcolor{capabilityTechnologyColor}}
\def\cellKD{\cellcolor{capabilityOrganizationColor}}

\def\cellMA{\cellcolor{measurementBusinessColor}}
\def\cellMB{\cellcolor{measurementDataColor}}
\def\cellMC{\cellcolor{measurementTechnologyColor}}
\def\cellMD{\cellcolor{measurementOrganizationColor}}

\newcommand{\labelKA}[1]{\cellKA \ref{sec:ekg-mm-a-#1} & \cellKA \nameref{sec:ekg-mm-a-#1}}
\newcommand{\labelKB}[1]{\cellKB \ref{sec:ekg-mm-b-#1} & \cellKB \nameref{sec:ekg-mm-b-#1}}
\newcommand{\labelKC}[1]{\cellKC \ref{sec:ekg-mm-c-#1} & \cellKC \nameref{sec:ekg-mm-c-#1}}
\newcommand{\labelKD}[1]{\cellKD \ref{sec:ekg-mm-d-#1} & \cellKD \nameref{sec:ekg-mm-d-#1}}

\newcommand{\taglineA}[1]{\cellMA \input{../../ekg-mm/fragments-capability-tagline/a-#1}}
\newcommand{\taglineB}[1]{\cellMB \input{../../ekg-mm/fragments-capability-tagline/b-#1}}
\newcommand{\taglineC}[1]{\cellMC \input{../../ekg-mm/fragments-capability-tagline/c-#1}}
\newcommand{\taglineD}[1]{\cellMD \input{../../ekg-mm/fragments-capability-tagline/d-#1}}

\begin{table}[ht]
    \centering\fontsize{7.2pt}{8.5pt}\selectfont
    \setlength\tabcolsep{2pt}
    \begin{tabular}{@{}cclcl@{}}
    \phantom{Pillar} & \textbf{Component} & & \textbf{Capability} & \textbf{Measurable ability} \\
    % A. Business Pillar
    \cellPA          & \cellCB            & \labelKA{1-1} & \taglineA{1-1} \\ % A.1 Business Strategy Actuation
    \cellPA          & \cellCB            & \labelKA{1-2} & \taglineA{1-2} \\
    \cellPA          & \labelCB{3}{1}     & \labelKA{1-3} & \taglineA{1-3} \\
    \cellPA          & \cellCB            & \labelKA{2-1} & \taglineA{2-1} \\ % A.2 Business Model Elaboration
    \cellPA          & \cellCB            & \labelKA{2-2} & \taglineA{2-2} \\
    \cellPA          & \labelCB{3}{2}     & \labelKA{2-3} & \taglineA{2-3} \\
    \cellPA          & \cellCB            & \labelKA{3-1} & \taglineA{3-1} \\ % A.3 Business Enablers
    \cellPA          & \cellCB            & \labelKA{3-2} & \taglineA{3-2} \\
    \cellPA          & \cellCB            & \labelKA{3-3} & \taglineA{3-3} \\
    \cellPA          & \cellCB            & \labelKA{3-4} & \taglineA{3-4} \\
    \labelPB{11}{a}  & \labelCB{5}{3}     & \labelKA{3-5} & \taglineA{3-5} \\
    % B. Data Pillar
    \cellPB          & \cellCD            & \labelKB{1-1} & \taglineB{1-1} \\ % B.1 Data Strategy
    \cellPB          & \labelCD{2}{1}     & \labelKB{1-2} & \taglineB{1-2} \\
    \cellPB          & \cellCD            & \labelKB{2-1} & \taglineB{2-1} \\ % B.2 Data Architecture
    \cellPB          & \cellCD            & \labelKB{2-2} & \taglineB{2-2} \\
    \cellPB          & \cellCD            & \labelKB{2-3} & \taglineB{2-3} \\
    \cellPB          & \labelCD{4}{2}     & \labelKB{2-4} & \taglineB{2-4} \\
    \cellPB          & \labelCD{1}{3}     & \labelKB{3-1} & \taglineB{3-1} \\ % B.3 Data Quality
    \cellPB          & \cellCD            & \labelKB{4-1} & \taglineB{4-1} \\ % B.4 Data Governance
    \cellPB          & \cellCD            & \labelKB{4-2} & \taglineB{4-2} \\
    \cellPB          & \cellCD            & \labelKB{4-3} & \taglineB{4-3} \\
    \labelPD{11}{b}  & \labelCD{4}{4}     & \labelKB{4-4} & \taglineB{4-4} \\
    % C. Technology Pillar
    \cellPC          & \labelCT{1}{1}     & \labelKC{1-1} & \taglineC{1-1} \\ % C.1 Technology Strategy
    \cellPC          & \cellCT            & \labelKC{2-1} & \taglineC{2-1} \\ % C.2 Technology Execution
    \cellPC          & \cellCT            & \labelKC{2-2} & \taglineC{2-2} \\
    \cellPC          & \labelCT{3}{2}     & \labelKC{2-3} & \taglineC{2-3} \\
    \cellPC          & \cellCT            & \labelKC{3-1} & \taglineC{3-1} \\ % C.3 User Interface
    \cellPC          & \cellCT            & \labelKC{3-2} & \taglineC{3-2} \\
    \labelPT{7}{c}   & \labelCT{3}{3}     & \labelKC{3-3} & \taglineC{3-3} \\
    % D. Organization Pillar
    \cellPD          & \cellCO            & \labelKD{1-1} & \taglineD{1-1} \\ % D.1 Executive Leadership
    \cellPD          & \cellCO            & \labelKD{1-2} & \taglineD{1-2} \\
    \cellPD          & \cellCO            & \labelKD{1-3} & \taglineD{1-3} \\
    \cellPD          & \cellCO            & \labelKD{1-4} & \taglineD{1-4} \\
    \cellPD          & \cellCO            & \labelKD{1-5} & \taglineD{1-5} \\
    \cellPD          & \labelCO{6}{1}     & \labelKD{1-6} & \taglineD{1-6} \\
    \cellPD          & \cellCO            & \labelKD{2-1} & \taglineD{2-1} \\ % D.2 Product Ownership
    \cellPD          & \cellCO            & \labelKD{2-2} & \taglineD{2-2} \\
    \cellPD          & \cellCO            & \labelKD{2-3} & \taglineD{2-3} \\
    \cellPD          & \cellCO            & \labelKD{2-4} & \taglineD{2-4} \\
    \cellPD          & \labelCO{5}{2}     & \labelKD{2-5} & \taglineD{2-5} \\
    \cellPD          & \cellCO            & \labelKD{3-1} & \taglineD{3-1} \\ % D.3 EKG Delivery Management
    \cellPD          & \cellCO            & \labelKD{3-2} & \taglineD{3-2} \\
    \cellPD          & \cellCO            & \labelKD{3-3} & \taglineD{3-3} \\
    \cellPD          & \cellCO            & \labelKD{3-4} & \taglineD{3-4} \\
    \cellPD          & \labelCO{5}{3}     & \labelKD{3-5} & \taglineD{3-5} \\
    \cellPD          & \cellCO            & \labelKD{4-1} & \taglineD{4-1} \\ % D.4 Organizational Culture
    \cellPD          & \cellCO            & \labelKD{4-2} & \taglineD{4-2} \\
    \cellPD          & \labelCO{3}{4}     & \labelKD{4-3} & \taglineD{4-3} \\
    \cellPD          & \cellCO            & \labelKD{5-1} & \taglineD{5-1} \\ % D.5 Organizaitonal Capabilities
    \cellPD          & \cellCO            & \labelKD{5-2} & \taglineD{5-2} \\
    \cellPD          & \cellCO            & \labelKD{5-3} & \taglineD{5-3} \\
    \labelPO{23}{d}  & \labelCO{4}{5}     & \labelKD{5-4} & \taglineD{5-4}
    \end{tabular}\label{tab:ekgmm-structure}
\end{table}

\egroup
